...
public class ForgetPasswordActivity extends AppCompatActivity implements View.OnClickListener, TextWatcher {
    private EditText regist_phonenumber,regist_password,regist_password2,regist_code;
    private Button code_bn,regist_bn;
    private String phonenumber,password,code,password2;
                            ...
                            if (response.isSuccessful()) {
                                ...
                                for (ValidCode mcode :validCodes){
                                    if (mcode.getPhonenumber().equals(phonenumber)&&mcode.getValidcode()==Integer.parseInt(code)){
                                        if (checkPassword(password)&password.equals(password2)){
                                            //开启动画
                                            handler.sendEmptyMessage(PROGRESSDIALOG);
                                            //满足注册条件进行注册
                                            Request request2 = new Request.Builder().url("http://192.168.1.112:9000/cloud/user/updatePassword?"+"password="+password+"&&"+"phoneNumber="+phonenumber).get().build();
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        ...
        //regist
        if (!regist_phonenumber.getText().toString().isEmpty()&&
